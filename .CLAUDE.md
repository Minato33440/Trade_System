...
# 更新: 2026-04-18（週次ワークフローVault参照追加）
...
STEP 4: 不明点があれば Vault の設計文書を参照
  ...
  STEP 4b (過去判断・バグ): Vault の bug_patterns/ または decisions/ を参照
  STEP 4c (大量ソース検索): @notebooklm-mcp にクエリ
...
⚠️ 設計文書の新規作成・更新時:
  1. docs/ の旧版を logs/docs_archive/ に移動する（必須）
  2. 日付なしファイル名で新版を docs/ に作成する
  3. docs/ に同名ファイルの旧版を残さない
```
...
```
週末のGitデータ更新作業時は、以下のVaultファイルを参照して全工程を実行する。

参照先: C:\Python\REX_AI\REX_Brain_Vault\wiki\trade_system\weekly_workflow.md

ボスからの入力データ:
  logs/gm/boss's-weeken-Report/2026/wr-YYYY-M-D.txt  ← 週次市況解説
  logs/gm/boss's-weeken-Report/trade_summary_tmp.txt  ← トレード履歴

出力先:
  logs/gm/weekly/2026/YYYY-M-D_wkNN/                  ← 週次フォルダ一式

⚠️ WEEKLY_UPDATE_WORKFLOW.md（docs/内の旧版）ではなく
   Vault の weekly_workflow.md を参照すること。
```
...
新機能: "Feat: ..."
ドキュメント: "Docs: ..."
週次更新: "weekly: YYYY-M-D_wkNN review + trade_results + charts"
```
