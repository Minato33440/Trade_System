...
# 作成: Rex / 最終更新: 2026-03-25
# 保存先: REX_Trade_System/docs/EX_DESIGN_CONFIRMED-2026-3-25.md
...
## 3. ミナト流MTF短期売買ルール（確定定義・2026-03-25版）
...
### 3-2. MTF 階層スキャン構造（#020検証完了・#021実装予定）
...
LAYER 3 — 窓内 15M/5M スキャン（#021実装予定）
  窓内 5M → 15M リサンプル
  → check_15m_range_low() で DB/IHS 判定（既存関数流用）
  → 15M ネック越え 5M 実体確定 → エントリー
```

#### エントリーロジック 2段階実装計画（2026-03-25確定）

```
Phase 1（#021 — シンプル版）:
  窓内 15M DB/IHS ネック越え → 5M実体確定 → エントリー
  → バックテスト → ベースライン数値を確定
...
│
├── window_scanner.py       🔄 #021で新規作成予定
│    窓ベース階層スキャン（Phase 1: シンプル版）
│    4H→1H窓→窓内15M DB/IHS→5Mネック越え
│    既存ファイルを一切変更せず独立動作
│
...
docs/
├── EX_DESIGN_CONFIRMED-2026-3-25.md  ← 本ファイル
├── PLOT_DESIGN_CONFIRMED-2026-3-25.md
...
├── 1h_windows/        ← 1H26本ウィンドウ + 5M重ね合わせPNG（8枚・修正済み）
└── window_scan_entries.csv  ← #021出力予定
```

---

## 5. Swing検出パラメータ（確定値・2026-03-25時点）
...
**#021（window_scanner Phase 1）の結果は別行で追記予定。**
...
| #020 | 1H-4H一致検証 + 窓プロット | test_1h_coincidence(v2) / plot_1h_window_5m | ✅ **完了** |
| #020-fix | プロットレンダリングバグ修正 | addplot→scatter / CJK→英語タイトル | ✅ **完了** |
| #021 | 窓ベース階層スキャン Phase 1 | window_scanner.py 新規作成 | 🔄 **指示書発行済み** |
| #022 | Phase 2: 15M右肩内5M DB | window_scanner.py 拡張 | ⬜ #021結果後 |

---

## 8. #020 完了結果（2026-03-25確定）
...
### 🔴 #021（最優先）— 窓ベース階層スキャン Phase 1

```
window_scanner.py 新規作成
  Layer 1: 4H LONG 期間 → 4H SL ts 取得
  Layer 2: 1H窓確定（前20+SL+後5=26本）
  Layer 3: 窓内15M→check_15m_range_low()→5Mネック越え確定

指示書発行済み・ClaudeCode実装待ち
完了条件: エントリー検出数・パターン別集計のCSV出力
```

### 🟡 #022（#021結果後）— Phase 2: フル版エントリー

```
15M DB/IHS 右肩検出 → 右肩内で 5M DB ネック実体上抜け
Phase 1 の結果をベースラインとして改善幅を定量比較
```

### 🟡 #023（#022後）— 決済シミュレーション統合

```
window_scanner.py に manage_exit() を統合
損益・PF・勝率を旧版 backtest.py と比較
```
...
詳細は `REX_Trade_System/docs/PLOT_DESIGN_CONFIRMED-2026-3-25.md` を参照。
