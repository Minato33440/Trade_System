...
# 日付なし・常に最新版として運用
# governance 正本: Trade_System\AGENTS.md（本ファイルは現状スナップショット）

---

## 本ファイルの位置づけ

Trade_System リポジトリの**現状スナップショット**。新規セッションの引き継ぎ時に最初に読む文書。
実装ロジックの詳細は `EX_DESIGN_CONFIRMED.md`・過去の判断経緯は `ADR.md` と
`Base_Logic/MTF_INTEGRITY_QA.md` を参照。

**実装ロジックの現在地**: #026d（PF 4.54 / 勝率 60% / +150.6p）。src/ 構造再編（責務分類・
archive 隔離・Trade_Brain 移設）を経てもロジック数値は不変。
...
|---|---|
| 最終判断 | Minato（ボス） |
| governance 正本 | `Trade_System\AGENTS.md`（源1枚・全engine収束） |
| engine 構成 | Hermes Runtime-Hub（grok / codex / claude gateway ＋ ClaudeCode-native） |
| リポジトリ | GitHub Minato33440/Trade_System |
| 役割分担 | 明示的な固定role分担は持たない。作業ごとにボスが差配し、engineは AGENTS.md に従う |
...
```
Trade_System/
├── CLAUDE.md                     # ClaudeCode-native harness 用シム（@AGENTS.md）
├── AGENTS.md                     # governance 正本（源1枚）
├── .env                          # API キー等（機密）
├── .gitignore
├── .venv/                        # Python 仮想環境
├── main.py
...
├── PLOT_DESIGN_CONFIRMED-2026-3-31.md  # プロット設計
│
...
> docs/ 直下には他に BRANCH_MAP / REMOTE_CONTROL / SIGNALS_SHORT_ANALYSIS /
> Trade_System_Wiki_Architecture / RTK / LOGIC_CONSISTENCY_MATRIX 等の旧地層ファイルが
> 残存。archive 判断は層B整理の残課題（maintenance_log 参照）。
...
| ファイル | 拡張範囲 |
|---|---|
| window_scanner.py | カラム追加・出力拡張が主。スキャンロジックは #026d 確定構造 |
| exit_simulator.py | 4 段階決済（方式B）。stage2/stage3 は D-12/D-13 訂正待ち |
...
```
F-5（設計判断優先順位）の 5 項目を確認
原則α（シンプルな土台の保守）に反していないか? → 反していれば却下
原則β（ノーリスク化後は伸ばさない）に反していないか? → 反していれば Phase 4 相当
原則γ（導入タイミングは安定性従属）を満たすか? → 満たさなければ保留
裁量思想（MINATO_MTF_PHILOSOPHY）と対応するか? → 対応しなければ MTF_INTEGRITY_QA で Q&A
```
...
| Phase | 内容 | 状態 |
|---|---|---|
| Phase 1 | 棚卸し・分類・Trade_Brain 移設・archive 隔離 | ✅ 完了 |
| Phase 2 | track_trades.py 隔離・plotter.py 共存方針確定 | ✅ 完了 |
...
（旧 REX_Trade_Brain (2d41d672-...) は MCP 接続切り離し済・廃止）
...
| Phase 3 責務別ディレクトリ化 | 未着手 | 起草後 |
| Phase 4 D-12/D-13 訂正 | 未着手 | Phase 3 完了後推奨 |
| docs/ 旧地層ファイル archive | 層B残課題 | maintenance_log 参照 |
...
本システムはミナトの裁量を「補助」するために構築されている。
一般的なトレード自動化ツール（全アルゴ判断・裁量排除）とは設計思想が根本的に異なる。
GMマクロトレードでは、タイムラグより「なぜ今エントリーするか」の判断品質が収益の主因となるため、
裁量をアルゴで置き換えるのではなく、**裁量の質を高めるためにシステムを使う**方針をとる。

### 進化の段階的方向性

| 段階 | 状態 | 内容 | トリガー |
|---|---|---|---|
| 現在 | 進行中 | 裁量補助ツール（バックテスト構築） | — |
| 中期 | 未着手 | データ蓄積によりアルゴ化できる領域を統計的に特定 | デモトレード稼働開始後 |
| 将来 | 未着手 | 特定された領域のみ段階的にアルゴ化・裁量領域は人間が保持 | Live稼働安定後 |

### 方針メモ（随時追記）

- Trade_Brainの週次蓄積は「どこをアルゴ化できるか」を発見するためのデータとして機能する
- 部分アルゴ化の判断はボスが行う

### 人的リソース拡張：スマートマネー・ギャップ抽出レイヤー（アイディア段階・2026-04-26）

GMマクロ戦略の精度向上に向け、市況ナラティブと「行動データ」の乖離を抽出する補助レイヤーのアイディア。
ヘッドラインは最も信頼性が低い情報源であるため、スマートマネーの**実際の売買行動**を参照することでギャップを可視化する。

**ギャップ抽出の構造**

ミナトのGM市況ナラティブ（Trade_Brain週次）
↕ ギャップ抽出
スマートマネーの実際の行動（以下ソース）
...
| ソース | 追う情報 | API | コスト |
|---|---|---|---|
| Quiver Quantitative | 議員売買・ロビー活動・政府契約 | ✅ Python公式パッケージ | $10〜/月 |
| Unusual Whales | 大口オプションフロー・ダークプール | ✅ REST API + MCP対応 | $57〜/月 |
| SEC EDGAR | インサイダーForm4・13F機関保有変化 | ✅ 完全無料 | 無料 |
| Finviz | スクリーナー・SEC連携 | ✅ MCP接続済み | Elite要 |
| Capitol Trades | 議員売買（UI特化） | ❌ 公式APIなし | スクレイピングのみ |

**運用方針（暫定）**

- ミナトの1次市況で確信度が低い局面に絞ってトリガー参照する
- 参照結果はTrade_Brain週次GM Strategyの「Smart Money乖離チェック」項目として記録
- 自動化前に手動チェックのフレームを固めてから実装判断する
- 現状テスト段階なのでボスが判断・執行する

**現在の実装状況**

- finviz MCP: Claude Desktopに接続済み（2026-04-26）・SEC EDGAR連携テスト待ち
- 次ステップ: finviz経由でインサイダーForm4・13F変化をwk04ナラティブと照合するテスト

---
*現状スナップショット。role分担記述は現行構成（役割固定なし・ボス最終判断・AGENTS.md 正本）に一元化済み。*