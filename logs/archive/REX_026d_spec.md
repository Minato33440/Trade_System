...
# 承認: ボス・Rex-Evaluator（Opus）（2026-04-15）
# think hard
...
```
① src/window_scanner.py — フィルター 3 行追加のみ
```
...
**実装ポイント**:
- `neck_1h` は None ガードで既にローカル変数として定義済み → 再取得不要
- 新しいカウンタ変数は不要（SKIP ログで件数は追跡できる）
- CSV カラム構成・プロット生成ロジックへの変更なし
- exit_simulator.py への変更なし（新 CSV 読込で自動反映）
...
# → SKIP(4H優位性なし) ログが少なくとも 1 件以上出力されること
...
01 | ...       | ...      | ...      | ...     | ...     | ...    | ...    | ...
（全件 neck_4h >= neck_1h であること）
```

---

## 設計根拠メモ（ADR D-10 向け・Evaluator wrap-up 時に統合）

※ Evaluator 採番確定（2026-04-15）: D-8=方式B迂回 / D-9=ENTRY_OFFSET / **D-10=本フィルター**

```
D-10: 4H-SwgH < 1H-SwgH によるエントリー不正検出（#026d）

症状: stage2_breakeven_stop で PnL がマイナス（#026c #07 型）
      neck_4h < entry_price（半値決済ターゲットがエントリー価格以下）

原因: 4H neck ブレイク後の下長髭裏確認パターン
      neck_4h（旧 4H SH）はブレイクされた水準にある
      → entry より下の可能性 → neck_4h < neck_1h（MTF 逆転状態）

修正: neck_4h < neck_1h の場合はエントリー除外
      = 4H-SwgH の値幅 >= 1H-SwgH の値幅（sl_4h ≈ sl_1h により等価）

教訓: neck_4h が半値決済として機能するには neck_4h >= neck_1h が前提
      ADR F-1（トップダウン原則）のエントリー検証への直接応用
      追加パラメータゼロ・チューニング不要
```
...
**Evaluator 承認: 2026-04-15（ADR D-10 番号適用済み）**
